...
class CompleteAutomationWorkflowSystem:
    ...
    def update_portfolio_articles_json(self, article_info: Dict) -> bool:
        ...
        try:
            ...
            # 新記事をblogArticlesの最上位に挿入（画像は手動対応）
            new_article = {
            ...
            # ポートフォリオサイトで画像自動取得を実行
            success = self.execute_portfolio_image_generation(article_info["url"])
            if success:
                print("📸 画像自動取得・保存完了")
            else:
                print("⚠️ 画像自動取得に失敗しました。手動で設定してください")
            
            # 投稿前確認システムの実行（オプション）
            print("🔍 投稿前確認システムを実行しますか？")
            print("  [y] はい（SEO品質確認）")
            print("  [n] いいえ（スキップ）")
            
            choice = input("選択してください (y/n): ").lower().strip()
            if choice == 'y':
                self.execute_pre_publish_confirmation(article_info)
    ...
    def execute_portfolio_image_generation(self, article_url: str) -> bool:
        """
        ポートフォリオサイトで画像自動取得を実行
        ...
        try:
            ...
            result = subprocess.run([
                'node', 
                f'{self.portfolio_path}portfolio_image_manager.js', 
                'process-article', 
                article_url
            ], capture_output=True, text=True, cwd=self.portfolio_path)
            
            if result.returncode == 0:
                print("✅ 画像自動取得成功")
                return True
            else:
                print(f"❌ 画像自動取得エラー: {result.stderr}")
                return False
                
        except Exception as e:
            print(f"❌ 画像自動取得実行エラー: {e}")
            return False
    
    def execute_pre_publish_confirmation(self, article_info: Dict) -> bool:
        """
        投稿前確認システムの実行
        ...
        try:
            ...
            article_data = {
                ...
                'meta_description': article_info.get('description', ''),
                'tags': article_info.get('tags', []),
                'slug': article_info.get('slug', ''),
                'category': article_info.get('category', 'その他'),
                'main_keyword': article_info.get('main_keyword', '')
            }
            ...
            if validation_result['overall_valid'] or validation_result['overall_score'] >= 60:
                ...
                if confirmed:
                    ...
                    print(f"✅ 投稿前確認完了: {saved_path}")
                    return True
                else:
                    print("📝 投稿確認がキャンセルされました")
                    return False
            else:
                print("❌ SEO品質基準に達していません")
                return False
                
        except Exception as e:
            print(f"❌ 投稿前確認システムエラー: {e}")
            return False

...
class WordPressBlogAutomator:
    ...
    def __init__(self, site_url: str = None, username: str = None, password: str = None):
        ...
        Args:
            site_url: WordPressサイトのURL（省略時は環境変数から読み込み）
            username: WordPress管理者ユーザー名（省略時は環境変数から読み込み）
            password: WordPressアプリケーションパスワード（省略時は環境変数から読み込み）
        """
        # 環境変数から読み込み（引数が指定されていない場合）
        ...
        if not all([self.site_url, self.username, self.password]):
            raise ValueError("WordPress認証情報が不足しています。環境変数または引数で指定してください。")
